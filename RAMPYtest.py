...
print( templates_path, templates )
...
if __name__ == "__main__":
    ...
    photo2.extend(["/work/pacicco/grackle/grackle_data_files/input/CloudyData_UVB=HM2012.h5"]*3)

    def quoted(string): return f"'{string}'"

    sbatch = sbatcher("myjob")
    
    sbatch.read(templates_path+"/"+templates[0])    
    sbatch.umodule("ompi")
    sbatch.lmodule("ompi/intel/2.1.1")	

    mynamelist = namelist("readRAMSES")
    mynamelist.read(templates_path+"/"+templates[1])
    ...
    for jn, dir, b0, ph0, ph1,ph2 in zip(job_names, dirs, bfields, photo0,photo1,photo2):
        nml_name = f"test_nml_{dir}.nml"
        sbatch_name = f"test_sbatch_{dir}.job"
