...
parser.add_argument("-c","--color",help ="Color of the line/s.",    default = ["midnightblue"],    nargs="+")
...
parser.add_argument("-l","--label",help ="Label/s for the hist/s.", default = None,       nargs="+",)
...
if args.n2 is None: 
    args.n2 = []
    for nn, nargs in enumerate(args.n1):
 
        args.n2+=[int(args.n1[nn])]
for nn, nargs in enumerate(args.n2):
    if  args.n1[nn] > args.n2[nn]: 
        n2tmp	= int(args.n2[nn])
        args.n2[nn] = int(args.n1[nn])
        args.n1[nn] = n2tmp
    ...
    for i in range(args.n1[j],args.n2[j]+1,args.step):
        ...
        if not args.single_plot: 
            ...
            try:
                ...
                print(error)    
            name = args.dir + "/tmp/" + args.x+ "_%05d"%i
            fig.savefig(name)
             
        k += 1
        first=False
name = args.dir +"/" + args.x +"_total_%05d-%05d"%(min(args.n1),max(args.n2))
if args.output is not None: name = args.output
